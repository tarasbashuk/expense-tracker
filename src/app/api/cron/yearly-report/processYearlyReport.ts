...
export async function processYearlyReportForUsers(users: User[]) {
  ...
  for (const user of users) {
    try {
      ...
      if (transactions.length === 0) {
        console.log(
          `No transactions for user ${user.email}, skipping yearly report`,
        );
        continue;
      }
      ...
      await sendYearlyReportEmail({
        ...
        defaultCurrency: user.settings?.defaultCurrency ?? undefined,
      });
